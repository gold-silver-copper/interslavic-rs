    ...
    for row in rows {
        for word in split_words(&row.isv) {
            ...
            if is_noun(pos) {
                ...
                adjectives.entry(word.clone()).or_insert(WordEntry {
                    word,
                    addition: row.addition.clone(),
                    part_of_speech: row.pos.clone(),
                });
            } else if pos.starts_with("v.") || pos.contains(" v.") {
                verbs.entry(word.clone()).or_insert(WordEntry {
                    word,
                    addition: row.addition.clone(),
                    part_of_speech: row.pos.clone(),
                });
...
fn generate_dictionary(
    ...
    out.push_str("#[derive(Debug, Clone, Copy, PartialEq, Eq)]\npub(crate) struct VerbEntry { pub word: &'static str, pub addition: &'static str, pub part_of_speech: &'static str }\n\n");
    out.push_str("#[derive(Debug, Clone, Copy, PartialEq, Eq)]\npub(crate) struct AdjectiveEntry { pub word: &'static str, pub addition: &'static str, pub part_of_speech: &'static str }\n\n");

    out.push_str("pub(crate) static NOUNS: &[NounEntry] = &[\n");
    for n in nouns {
        out.push_str(&format!(
            "    NounEntry {{ word: \"{}\", addition: \"{}\", part_of_speech: \"{}\", gender: {}, animacy: {}, indeclinable: {}, singular_only: {}, plural_only: {} }},\n",
            escape(&n.word), escape(&n.addition), escape(&n.part_of_speech), n.gender, n.animacy, n.indeclinable, n.singular_only, n.plural_only
        ));
    }
    out.push_str("];\n\n");
    ...
    for v in verbs {
        out.push_str(&format!(
            "    VerbEntry {{ word: \"{}\", addition: \"{}\", part_of_speech: \"{}\" }},\n",
            escape(&v.word),
            escape(&v.addition),
            escape(&v.part_of_speech)
        ));
    }
    out.push_str("];\n\n");
    ...
    for a in adjectives {
        out.push_str(&format!(
            "    AdjectiveEntry {{ word: \"{}\", addition: \"{}\", part_of_speech: \"{}\" }},\n",
            escape(&a.word),
            escape(&a.addition),
            escape(&a.part_of_speech)
        ));
    }
    out.push_str("];\n");
    out
