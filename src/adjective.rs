...
impl Adjective {
    ...
    pub fn new(record: &ISVEntry) -> Self {
        let word = &record.isv;
        let boop = if word.ends_with("o") {replace_last_occurence(word, "o", "y")}
  else if word.ends_with("e") {replace_last_occurence(word, "e", "i")}
  else {word.to_string()};
  ...
  Adjective{
    ...
    pub fn basic_declension(word:&str,g:&Gender,c:&Case,n:&Number, animate:bool) -> String {
        ...
        let ending = match g {
            ...
            Gender::Masculine=>match n{
                Number::Sing=> match c{
                    Case::Nom=>    format!("{yi}") ,
                    Case::Acc=> if animate {format!("{oe}go")} else {format!("{yi}")},
                ...
                Number::Plur=> match c{
                    Case::Nom=>if animate {format!("i")} else {format!("e")},
                    Case::Acc=>if animate {format!("{yi}h")} else {format!("e")},
                    Case::Gen=>format!("{yi}h"),
                    Case::Dat=>format!("{yi}m"),
                    Case::Ins=>format!("{yi}mi"),
                    Case::Loc=>format!("{yi}h"),
                    Case::Voc=> if animate {format!("i")} else {format!("e")},
                },
            },
    ...
    pub fn adverb(&self) -> String {
        ...
        Adjective::basic_declension(&self.base_form, &Gender::Neuter, &Case::Nom, &Number::Sing, true)
    }
    ...
    pub fn synthetic_comparative(&self) -> String {
        ...
        let ending = if adj_stem.ends_with("eky") || adj_stem.ends_with("oky") {"ši"}
        else if adj_stem.ends_with("ky") && is_consonant(last_in_slice(&slice_without_last(&adj_stem))) {
